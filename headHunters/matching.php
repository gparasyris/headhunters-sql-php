            ...
            <div class="wrapper group">
                ...
                <div id="content" class="layout-sidebar-right group">
                    ...
                    <div id="slogan" class="inner">
                        <h1 class="only">Here you can find several possible <span>matchings</span> between personnel and jobs.</h1>
                    </div>         
                    <!-- END SLOGAN --> 
                    ...
                    <?php
						...
						$query = MYSQL_QUERY("SELECT * FROM jobposition, isoffered WHERE available=1 AND jobposition.code=isoffered.jobCode");

						while($row = mysql_fetch_array($query)){
					?>
                        <div class="toggle">
                            <p class="tab-index tab-opened"><a href="#" title="Close"><?php echo $row[0].', '.$row[1]; ?></a></p>
                            <div class="content-tab opened">
                                <div class="arrow">&nbsp;</div>
                                <p>
                                    <?php
										MYSQL_QUERY("SET NAMES 'utf8'");
										$query2 = MYSQL_QUERY("SELECT customer.name, customer.phone, customer.email 
										FROM individual, customer, isrequested 
										WHERE individual.id = customer.id AND isrequested.individualId = individual.id AND individual.job='$row[1]' AND isrequested.availableFrom >= $row[9] AND unemployed=1 AND NOT EXISTS(
											SELECT diploma FROM jobpositionstudies WHERE code='$row[0]' AND NOT EXISTS
											(SELECT diploma FROM inidividualstudies WHERE id=individual.id)) AND NOT EXISTS(
											SELECT skill FROM jobpositionskills WHERE code='$row[0]' AND NOT EXISTS
											(SELECT skill FROM inidividualskills WHERE id=individual.id)) AND NOT EXISTS(
											SELECT foreignLanguage FROM jobpositionforeignLanguages WHERE code='$row[0]' AND NOT EXISTS
											(SELECT foreignLanguage FROM inidividualforeignLanguages WHERE id=individual.id))");
										while($row2 = mysql_fetch_array($query2)){
											echo $row2[0].', Phone: '.$row2[1].', e-mail: '.$row2[2].'<br/>';
										}
									?>
